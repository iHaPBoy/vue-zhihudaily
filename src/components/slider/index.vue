<template>
  <div class="slider" :class="{'slider-cursor': click}" @mouseover="pause && pausePlay()" @mouseout="pause && goPlay()"
    ...
    <div class="slider-item-list"
         :style="{'width': `${count * 100}%`, 'left': `${-100 * current}%`, transitionDuration: `${speed}s`}">
      <slider-item v-for="(item, index) in items" :key="index" :count="count" :item="item" :click="click"></slider-item>
    </div>
...
<script>
  ...
  import SliderArrows from './slider-arrows.vue'
  export default {
    data () {
      return {
        current: 0,
