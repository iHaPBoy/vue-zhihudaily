<template>
  <div class="news-view">
    ...
    <transition :name="transition">
      <div class="news-list" :key="displayDate" v-if="displayDate > 0">
  ...
  export default {
    ...
    methods: {
      loadItems (to = this.date, from = -1) {
        ...
        this.$store.dispatch('FETCH_DATE_ITEM_LIST_DATA', {date: this.date}).then(() => {
          ...
          this.displayDate = to
          this.displayItems = this.$store.getters.activeSimpleItems
          this.loading = false
        })
      }
    },
    data () {
      ...
      return {
        ...
        displayDate: isInitialRender ? this.date : -1,
        displayItems: isInitialRender ? this.$store.getters.activeSimpleItems : []
      }
    },
    computed: {
      ...mapGetters([
